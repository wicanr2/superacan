...
| U3 `UM70C188` | 8-bit palette data 轉 RGB 的三路 DAC／palette 介面；早期筆記稱與 `UM70C171` 同類 | 數位色彩必要；類比波形可近似 | `PPU.sch` 與早期板級筆記 (p)；沒有找到獨立公開模擬器 |
...
die tracing，但目前狀態為 **Stalled**，沒有可供重製直接使用的公開 netlist 或 emulator core。
