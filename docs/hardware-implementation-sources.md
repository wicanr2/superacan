...
| 卡帶 `UM6650` | 16-byte key ROM、32-byte RAM 與授權／lockout 輸出 | 開機必要 | 卡帶電路圖 (p)、IPL (a)、MAME device (b)；三方埠角色一致 |
