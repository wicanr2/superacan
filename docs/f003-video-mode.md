...
### 7.6 自製 ROM 讓該分支可達，並量到行為差異（2026-09-01）

既然商業 ROM 不會讓兩個條件同時成立，就自己做一顆。`homebrew/bit3probe/` 是一顆 512 KiB
自製卡帶：ROZ 以 8bpp region 運作，主迴圈每 180 幀把 `$F001F0` 在 `$0001`／`$0009` 之間
切換，並用調色盤第 255 號的顏色（紅／藍）作為相位標示。授權區 `$2000–$23FF`（八款流通
ROM 完全相同的 1024 bytes）於建置時由本機 ROM 取出，因此映像可通過 IPL 檢查，在 Bcan、
本專案模擬器與實機上跑同一段程式；產物含版權資料，只留本機。

同一顆映像的結果：

| 執行環境 | bit 3 = 0 | bit 3 = 1 |
|---|---|---|
| 本專案 Go renderer | 畫出 ROZ 圖樣 | 畫出 ROZ 圖樣（僅標示方塊變色） |
| Bcan 0.0.8b | **整片 backdrop（palette 0），ROZ 層不出現** | 畫出 ROZ 圖樣 |

即：在同一組 ROZ 暫存器設定下，**Bcan 只有在 bit 3 設起時才畫出 8bpp ROZ 層**。這是第一個
可重現、可觀察的 bit 3 行為，且來自自製軟體而非商業 ROM。它與 §7.3 的靜態結構一致——
`v263` 為真時走的是不經逐行表的路徑，而本 ROM 沒有設定逐行表暫存器，於是另一條路徑取到
全 0 的表、整層被判為不繪製。

尚未定案的是閘門的完整條件：把 `$F00182` bit 8 設為 1 的變體沒有改變結果，代表閘門不只
該位元，或該位元的位置判讀有誤。下一輪要把 palette 0 改成與相位標示不同的顏色（目前
backdrop 與 bit3=0 的標示同為藍色，靠推理才排除歧義），並逐一掃 `$F00180`／`$F00182`
的位元組合。
