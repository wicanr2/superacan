...
1. **整層改成線性 bitmap**：不再查 tilemap 與 tile 圖形，而是把 VRAM 當成一張連續點陣圖，
   逐像素直接取值（`1400A0138`–`1400A018A`）：

   ```text
   bit   = bpp × (x + width × y)              // width = 8 × 版面 tile 欄數
   addr  = ((bit >> 3) + 4 × $F00196) & 0x1FFFF
   pixel = (VRAM[addr] >> (bit & 7)) & ((1 << bpp) - 1)
         + (($F00182 & 0xF) << bpp)           // palette bank
   ```

   `mov ecx,[rsi+8Ch]` 取的是 snapshot 的 bitmap 基底欄位，`lea ecx,[r9+rcx*4]` 即 ×4；
   `$F00196` 在暫存器解碼時以原值存入（`sub_1400A9200` 的 `case 0x196`），因此基底就是
   該暫存器的四倍 byte 位址。像素值 0 視為透明。
...
在唯一會消費該位元的實作裡，bit 3 是 **ROZ 層的 tilemap／bitmap 切換**，不是全域
direct-color 開關；
...
### 7.6 自製 ROM 讓該分支可達，並解出它的實際語意（2026-09-01）

既然商業 ROM 不會讓兩個條件同時成立，就自己做一顆。`homebrew/bit3probe/` 是一顆 512 KiB
自製卡帶：ROZ 以 8bpp region 運作，主迴圈每 300 幀把 `$F001F0` 在 `$0001`／`$0009` 之間
切換，並把 backdrop（調色盤索引 0）在兩個相位分別設成紅與綠，因此單看一張截圖就能判斷
相位，即使整層沒有畫出來也不會誤判。授權區 `$2000–$23FF`（八款流通 ROM 完全相同的
1024 bytes）於建置時由本機 ROM 取出，映像因此能通過 IPL 檢查；產物含版權資料，只留本機。

ROM 另外把 tile 之後的整片 VRAM 填成索引 200，並讓 tilemap 不使用 tile 0，使 128 KiB
視窗內沒有任何零 byte。這樣「畫面空白」就只可能來自整層未繪製，不可能來自取到零值。

Bcan 的兩個相位是兩種完全不同的取值方式：

| `$F001F0` | Bcan 畫面 | 判讀 |
|---|---|---|
| `$0001`（bit 3 = 0） | tile 圖樣，色彩來自 tile 資料 | 一般 tilemap 路徑 |
| `$0009`（bit 3 = 1） | 整片索引 200，即 VRAM 填值本身 | 線性 bitmap 路徑 |

bitmap 基底以一次對照實驗定位：`$F00196` 由 `$0000` 改成 `$0800` 時，VRAM `$2000–$27FF`
（tilemap 所在、高位元組為零因而透明）造成的橫向條紋從畫面第 32–39 列移到第 0–7 列。
`0x2000 / 256 = 32`，與 §7.3 的 `4 × $F00196` 及 256 byte 每列跨距（版面 32 tile 寬）相符。

本機的 Linux 重製（`../superacan-emu`）依此實作 bitmap 路徑後，同一顆映像的兩個相位與
Bcan 的截圖**逐像素相同**（相異 0／76800、SHA-256 一致），因此這條路徑已有可重跑的
同畫面驗證，不只有靜態反組譯。

### 7.7 bit 4 沒有自己的作用（2026-09-02）

§7.3 的條件是 `pixel_mode` **恰為** `$08`，不是「bit 3 有設」。這代表 bit 4 一旦設起來，
bit 3 的 bitmap 路徑就失效。`homebrew/pixelmodeprobe/` 用同一個 ROZ 8bpp 場景把
`$F001F0` 在四個值之間循環，每 300 幀一次，直接量到：

| `$F001F0` | pixel mode | ROZ 走哪條路 | 畫面雜湊（前 10 碼）|
|---|---|---|---|
| `$02` | `$00` | tilemap | `99548f2e45` |
| `$0A` | `$08` | **bitmap** | `985c1c47fc` |
| `$12` | `$10` | tilemap | `a46b257b55` |
| `$1A` | `$18` | tilemap | `543f68c4eb` |

Bcan 與本專案重製產生的四張畫面完全相同（同一組雜湊）。三個 tilemap 相位的雜湊不同，
只是因為相位標示的顏色不同。

因此 `$F001F0` 的全部四個欄位現在都有定論：bits 0–2 是 gfx mode（與 MAME 相同的三張
region 表）、bit 3 是 ROZ 的 tilemap／bitmap 切換、**bit 4 沒有獨立效果，只作為 bit 3 的
排他條件**。這也讓 §7.4 對 direct-color 假說的評估更明確：兩個位元都沒有全域色彩效果。
