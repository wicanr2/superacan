...
| `$E80000–$E8FFFF` | 經函式指標（+300440）轉發 SoundHostPort 物件（host 端以 byte 陣列保存，word 存取在該物件內組合；對 65C02 可見的位元組序不變，見 sound-driver.md §0） |
...
| `$EB0D00–$EB0D03` | UMC6650：偶位址寫入忽略；`$EB0D03` = 位址埠（寫 +300432，7-bit）；`$EB0D01` = 資料埠。**讀**：位址埠 $20–$2F → 讀 16 byte 金鑰（+300384，即 umc6650.bin）；$40–$5F → 讀內部 RAM（+300400）。**寫**：僅當（位址埠 & 0xE0)==0x40 才寫入內部 RAM——**金鑰區 $20–$2F 唯讀**。與 IPL 反組譯結論（bios-68k.md §3）及 MAME device 一致 |
...
- `$E90004/05` 與 `$E9000C` 是 68k 讀取 65C02 交出的取樣 DMA 請求位址與旗標
  （分別對應 sound RAM `$040C/$040D` 與 `$040A`，SystemBus +5168/+5169/+5170），
  不是手把埠；手把由 65C02 shift register 掃描後放在 sound RAM `$0200/$0202`
  供 68k 讀取（memory-map.md §5、sound-driver.md §4.3）。
