...
## 2026-09-02：納入 1995 年的一手史料《A'can 特輯》

- 從私有的《電視遊樂雜誌》專藏複製七份史文進 `docs/history/`：敦煌科技、聯華電子、
  精訊資訊、全崴資訊、李培民與 1995 年編年史。**雜誌內文的 OCR 全文與掃描圖沒有複製**
  ——那是尖端出版與敦煌科技的著作，留在私有專藏；這裡只有史文與佐證所需的短引文。
- 新增 [docs/console-history.md](docs/console-history.md)：把 1995 年的規格宣稱逐條擺到
  已量測的結論旁邊。**講得出數字的四項全部相符**——背景 5 層對上 4 個一般圖層加 ROZ、
  256×256 對上 sprite 寬高上限、256 色對上 8bpp 調色盤、放大縮小與遠近景深對上
  `hscale`／`vscale` 與 ROZ 仿射變換。沒有找到互相矛盾的項目。
- 三件補到的事：
  - **「F-16」有了一手出處**。先前只有維基等 (c) 整理，現在有 1995 年兩篇專訪直接使用
    「F-16 遊戲平台」「A'CAN F-16」。外盒印的是「SUPER A'can」，三種寫法同期並存。
  - **開發期約始於 1992–1993**（自述「花了將近 3 年」回推），首度公開展示 1995-02-09。
  - **MAME catalog 的 `Kingformation` 就是精訊資訊有限公司**，1995 年 10 月的專訪拍到
    門牌英文社名作「…formation Co., Ltd.」，且該公司當時正在開發《超級光明戰史》。
    同篇訪談也交代了 F007 為何是 1996 年——原訂與主機同步發售，因人員流失（美工面臨
    兵役）延後。這條寫進 software-catalog.md §2.1。
- **一處改成並陳**：本庫原記敦煌科技是「聯華電子子公司」（(c) 來源）。1995 年的原廠
  專訪把兩者描述成技術合作（聯電做關鍵 IC、敦煌提供軟體 KNOW-HOW），沒有子公司的說法。
  兩者都是 (c)，CLAUDE.md 改成並陳不裁決，理由寫在 console-history.md §3。

### 分寸

- **帶廠商立場的一手材料仍然是 (c)。** 這冊的版權頁明印「本書圖文皆轉載於敦煌科技及
  各軟體廠商」，所以它對硬體的描述是廠商提供的資料而非雜誌的獨立測試。比維基近，
  但不是量測；與 (a) 衝突時仍以 (a) 為準。這層性質在 history/README.md 標在最前面。
- 原庫的 `companies/`／`people/` 分目錄攤平成單一目錄，跨目錄連結逐一改寫並驗過；
  站台重建後 280 個連結全數可解析。

## 2026-09-02：包絡模型獲得遊戲寫入值佐證

- 從三款遊戲的存檔狀態讀出 UM6619 的暫存器現值（`machine.ParseSaveState` 的
  `Audio.Registers`），用 §5.2 的公式換算成時間，四項對得上：
  - 每個音樂通道都寫 `$40 = $03`（包絡開關 bit1），Monopoly 的取樣串流通道 ch15 寫
    `$40 = $01` 且 `$90 = $FF`——正是模型要求的「串流通道不走包絡」。MAME 那句
    `Normally 0x03 for keyon channels, 0x01 for streaming DMAs` 的原因就是這個。
  - `$A0 = $1F`（跳過 attack）的特例被實際使用：12 組音色有 5 組是它，而且**每一組都
    同時是 `$B0 = 0`（不衰減）配短 release**——打擊樂的標準設定。欄位解錯不會有這種聚集。
  - 換算出的時間全落在音樂範圍：attack 12–44 ms、decay 114 ms–6 s、release 8 ms–2 s。
  - 同一組參數在多個通道重複（Boom Zoo 九個通道共用 `0E 01 06 57`），是音色定義套用的樣子。
- 一筆例外照實記錄：Monopoly ch6 的 release 換算成 64 秒，數值不合理。
- **佐證的是欄位語意，不是時間常數。** 500／6000／2500／8／256 這些係數換成別的比例會得到
  整體縮放不同但同樣「看起來合理」的結果，所以常數仍然只有 Bcan 一個來源。

**結案**：欄位語意定案（反編譯 ＋ 遊戲寫入值雙向支持）；時間常數與音量查表維持
`confirmed-Bcan`，已接受為現況不列進行中——判它需要實機。是否實作屬 superacan-emu 的
決策，不在本庫追蹤。

### 方法

- **靜態掃描先失敗才換路。** 先掃九款 ROM 找 `lda #imm; sta $0420` 這種立即值暫存器號，
  只撈到 33 筆而且全在 `$10–$1F`——暫存器號多半來自表，不是立即值。改從執行後的狀態讀，
  一次就拿到全部 16 通道的完整參數。
- **存檔狀態是現成的暫存器傾印。** 不必改模擬器加旗標（那會動到別人正在編輯的檔案），
  存檔裡本來就有 `[256]uint8` 的完整暫存器陣列。

## 2026-09-02：UM6619 的包絡與音量展開追出來（`$40`、`$A0–$DF`、`$E0`）

- **`$A0/$B0/$C0/$D0` 是每通道 ADSR 包絡的四個參數**，`$40` bit1 是開關。語意來自
  `sub_1400A7D80`（狀態進入）與 `sub_1400A8880`（每取樣更新），暫存器對應由
  `sub_1400A4190`（寫入分派）確認：attack 線性、decay 平方曲線、第二段衰減與 release
  是幾何衰減，增益 24 位元小數乘在音量之後。逐欄位表寫入 sound-driver.md §5.2。
- **key-off 不是立即停聲**：`$17` 高 nibble 為 0 時 Bcan 進 release 態，只有狀態 6 才真的
  清 active 位元。MAME 直接清位元，兩者尾音處理完全不同。
- **`$E0` 的音量 nibble 展開兩邊不同**：Bcan 用一張非線性 16 項查表
  （`0 51 68 102 …`），MAME 是線性 ×17，只有頭尾相同。哪一版對應實機待查證。
- 取樣串流通道寫 `$40 = $01`（bit1 清除）不走包絡，音樂通道寫 `$03`——與 MAME 原始碼
  註解「Normally 0x03 for keyon channels, 0x01 for streaming DMAs」完全吻合，
  是這個模型的第一個獨立佐證。
- 本專案三項都沒有實作。下一步不是照抄，而是先掃遊戲驅動實際寫進 `$A0–$DF` 的值，
  看在這個模型下是否有音樂上的意義；有意義才值得實作。

### 方法

- **從存檔驗證函式讀版面最省力。** `sub_1400A89D0` 為了驗證還原後的狀態一致，
  把「每個通道欄位該等於哪個暫存器」逐條寫了出來——等於一份免費的欄位對照表，
  比逐一追寫入路徑快得多。
- 找暫存器語意的錨點順序：**查表常數 → xref → 寫入分派**。這次是先 dump 音量表
  `byte_140424FBB`，再用它的 xref 直接落到寫入處理常式。

## 2026-09-02：Bcan 的音訊管線，以及一個被推翻的斷言

- **輸出走 WINMM 的 `waveOut`**（不是 XAudio2／WASAPI）：PCM 立體聲 48000 Hz、16-bit，
  18 個 `WAVEHDR` 各 1024 byte（256 取樣、5.33 ms），佇列深度約 96 ms，前面接一個
  0x2000 取樣的環形緩衝。Media Foundation 只用在 MP4 錄影。
- **合成在 44744 Hz（`clock/80` 取整）**，再由 44744 → 48000 的重取樣器轉換，
  `sub_14009AD80` 對這組數字有寫死的快速路徑。
- **混音器多了一層 MAME 沒有的包絡產生器**：取樣轉換與音量算式與 MAME 逐值等價，
  但通道狀態不為 0 時左右聲道再乘一個 24 位元小數增益，且每取樣呼叫 `sub_1400A8880`
  更新——線性漸變、平方漸變、幾何衰減三種曲線的狀態機。MAME 的裝置有 `envelope[4]`
  欄位卻從未使用。本專案跟隨 MAME，因此**沒有這一層，音色會與 Bcan 有差**。
- **推翻**：先前由回讀掃描得出的「Bcan 對 254 個編號只是單純存放、沒有語意，因此不能
  當 `$40–$4E`／`$A0–$DF` 的 oracle」不成立。回讀原樣只證明值被存下來，不排除副作用；
  包絡狀態機就是那個副作用。`docs/sound-driver.md` §6.1 已改寫成現況。

### 教訓

- **回讀值原樣 ≠ 寫入沒有副作用。** 用「寫進去再讀回來」掃描暫存器時，只能得到
  「這個手段看不到語意」，不能升級成「沒有語意」。要否定語意得看實作或行為，不是回讀。
- 查一個模擬器「怎麼做到某件事」時，**先看匯入表**比先反編譯便宜得多：`waveOut` 系列
  一出現，整條音訊路徑的形狀就定了，後面的反編譯只是填細節。

## 2026-09-02：DMA 通道 1、暫存器獨立性，與時序能說到哪裡

- **通道 1 實際搬移**：同一組 12 個案例改跑 `$E90030`，畫面與通道 0 的版本逐像素相同
  （Bcan 與本專案各自比對都是 0 個像素差）。通道 1 沒有自己的特殊行為。
- **兩個通道的暫存器檔是分開的**：先把通道 1 設好但不觸發，用通道 0 搬 8 個 byte，
  再只寫通道 1 的 control——第三步用的是通道 1 自己存下的位址與 count（目的區 0 出現
  16 個 byte、通道 0 的目的區仍是 8 個）。共用的話第三步會搬錯地方。
- **時序：查到可引用的參考，但沒有升格成 A'Can 的規格。** 分三層寫進
  [docs/host-dma.md](docs/host-dma.md) §6：
  - (p) `APU.sch` 上 UM6619 接 68000 的 `BR/BG/BGACK`，所以 DMA 是走三線仲裁把匯流排
    要走的 bus master，搬移期間 CPU 拿不到匯流排。這一層不需要類比。
  - (a/b) Bcan 與 MAME 都是觸發即完成。MAME 原始碼留了一行被註解掉的
    `spin_until_time(count * 2)`，並註明「沒看到任何軟體讀 DMA 狀態」。
  - (c) MC68450 DMAC 原廠 datasheet 當參考框架：dual-address 每單位兩個匯流排週期、
    「讀四個時脈寫五個時脈」、back-end overhead 最佳兩個時脈、模式分 burst／cycle steal／
    cycle steal with hold。套上 10.738635 MHz 得每單位約 838 ns，32 KiB 的 byte 搬移
    約 27 ms——超過一幀，所以「瞬間完成」在實機上不可能成立。
- 收斂方式也寫進去了：用計數器 `$E90014/16` 在觸發前後各讀一次量實際耗時，需要實機；
  或等到出現一款會因時序不同而表現不同的軟體。目前沒有反例，也沒有判準。

### 方法上的分寸

- **同一條匯流排上的原廠控制器可以當參考框架，不能當證據。** MC68450 是外接控制器，
  UM6619 把 DMA 做進系統控制器裡，週期數與 overhead 都可能不同。表格照抄進文件時
  逐列標明「對 A'Can 的意義」，並在表後明寫這不是 A'Can 的規格。
- **通道 1 的驗收條件是「兩張圖逐像素相同」**，不必另訂判準——把目的區與案例設成
  一模一樣，差異就只剩通道本身。
