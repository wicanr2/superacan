| Super A'Can homebrew：$F001F0 的 pixel mode（bits 4-3）四相位掃描
|
...
map_loop:
        ...
        | 相位表：每筆兩個 word（$F001F0 的值、backdrop 顏色）
        lea     phases,%a5
        moveq   #0,%d2                  | 幀計數
        moveq   #0,%d4                  | 相位索引 0..3
        bsr     apply_phase
...
| 依相位索引寫 $F001F0 與 backdrop；索引 255 固定白色作為 tile 圖樣的標示。
apply_phase:
        move.w  %d4,%d0
        lsl.w   #2,%d0                  | 每筆 4 byte
        movea.l #0x00F001F0,%a3
        move.w  0(%a5,%d0.w),(%a3)
        movea.l #0x00F00200,%a2
        move.w  2(%a5,%d0.w),%d1
        move.w  %d1,(%a2)               | backdrop
        move.w  %d1,0x1FE(%a2)          | 索引 255：地圖中央的標示方塊
        rts
